...
std::optional<std::variant<IPv4Address, IPv6Address>> DNSAddress::resolve() const
  {
    if (!dirty)
      return resolved_ip;
    ...
    hints.ai_socktype = 0; // or 0 if you do not care
    hints.ai_protocol = 0; // or 0 if you do not care
    ...
    std::optional<std::variant<IPv4Address, IPv6Address>> resolved;
    ...
    for (addrinfo* p = result; p != nullptr; p = p->ai_next)
    {
      if (p->ai_family == AF_INET)
      {
        ...
        resolved = IPv4Address(bytes[0], bytes[1], bytes[2], bytes[3]);
        break;
      }
      else if (p->ai_family == AF_INET6)
      {
        ...
        resolved =
            IPv6Address((static_cast<uint16_t>(bytes[0]) << 8) | bytes[1],
