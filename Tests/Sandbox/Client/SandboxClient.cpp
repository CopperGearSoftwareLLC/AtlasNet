...
void SandboxClient::RenderView()
{
    ...
    ImGui::SetNextWindowSizeConstraints(ImVec2(avail_space.x * 0.4f, 0), ImVec2(avail_space.x * 0.9f, avail_space.y));
    ImGui::BeginChild("game view", {0, 0});
    if (ImPlot::BeginPlot("view", ImVec2(-1, -1), ImPlotFlags_CanvasOnly | ImPlotFlags_NoFrame | ImPlotFlags_NoTitle | ImPlotFlags_NoInputs|ImPlotFlags_Equal))
    {
        static  auto plot_size = ImVec2(1,1);
        const float Aspect_Ratio = plot_size.y / (float)plot_size.x;
        ImPlot::SetupAxesLimits(CameraPos.x - CameraSizeX, CameraPos.x + CameraSizeX, CameraPos.y - CameraSizeX * Aspect_Ratio, CameraPos.y + CameraSizeX * Aspect_Ratio,ImGuiCond_Always);
        ...
        ImPlot::GetPlotDrawList()->AddCircleFilled(ImPlot::PlotToPixels( ImVec2(CameraPos.x,CameraPos.y)),100.0f * 1/CameraSizeX,ImGui::ColorConvertFloat4ToU32(ImVec4(0,0,1,1)));
        ImPlot::EndPlot();
    }
    ImGui::EndChild();
    ImGui::End();
...
int sandbox_client_entry(int argc, char **argv)
{
    SandboxClient cl;
