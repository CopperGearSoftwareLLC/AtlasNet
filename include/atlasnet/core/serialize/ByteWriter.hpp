...
public:
  ...
  std::string as_string_base_64() const {
    ...
    const size_t written_count = boost::beast::detail::base64::encode(
        out.data(), buf.data(), buf.size());

    AN_ASSERT(written_count > 0, "BASE64 encode did not write");
    return out;
  }
  ...
  ByteWriter &f32(float v) { return u32(std::bit_cast<uint32_t>(v)); }

  ByteWriter &f64(double v) { return u64(std::bit_cast<uint64_t>(v)); }

  template <typename T> ByteWriter &write_scalar(T v) {
  ...
  template <uint32_t Dim, typename T> ByteWriter &write_vector(const T &v) {
    for (uint32_t d = 0; d < Dim; ++d)
      write_scalar(v[d]);
    return *this;
  }

  ByteWriter &vec2(const glm::vec2 &v) { return f32(v.x).f32(v.y); }

  ByteWriter &vec3(const glm::vec3 &v) { return f32(v.x).f32(v.y).f32(v.z); }

  ByteWriter &vec4(const glm::vec4 &v) {
    return f32(v.x).f32(v.y).f32(v.z).f32(v.w);
  }

  ByteWriter &ivec2(const glm::ivec2 &v) { return i32(v.x).i32(v.y); }

  ByteWriter &ivec3(const glm::ivec3 &v) { return i32(v.x).i32(v.y).i32(v.z); }
  ...
  ByteWriter &uuid(const UUID &id) { return write(id.data(), id.size()); }
  ...
  ByteWriter &var_i32(int32_t v) { return var_u32((v << 1) ^ (v >> 31)); }
...
private:
  template <typename T> void push_be(T v) {
