...
if [[ -z "${WORKER_IPS// }" ]]; then
  echo "No workers configured (WORKER_IPS/PI_WORKER_IP is empty); skipping worker SSH setup."
else
  for ip in $WORKER_IPS; do
    ensure_key_access "$WORKER_SSH_USER" "$ip" "worker ${ip}"
  done
fi

echo "SSH setup done. Run 'make linux' (or legacy alias 'make linux-pi')."
