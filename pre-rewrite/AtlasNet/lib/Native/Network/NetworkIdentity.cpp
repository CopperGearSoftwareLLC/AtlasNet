...
NetworkIdentity NetworkIdentity::MakeIDProxy(const UUID &_ID)
{
	NetworkIdentity id;
	id.Type = NetworkIdentityType::eProxy;
	id.ID = _ID;
	return id;
}
...
std::optional<NetworkIdentity> NetworkIdentity::FromEncodedByteStream(const std::array<std::byte,
32> &input)
{
...
std::optional<NetworkIdentity> NetworkIdentity::FromEncodedByteStream(const std::byte *data, size_t
size)
{
