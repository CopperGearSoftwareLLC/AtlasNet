...
{
	return InterLinkIdentifier(InterlinkType::eShard, DockerIO::Get().GetSelfContainerName());
}
void Partition::Init()
{
		...
		.callbacks = {
				...
				[this](const InterLinkIdentifier &Connection)
			{
				...
				if (Connection.Type == InterlinkType::eGameServer)
				{
					...
					logger->DebugFormatted("[Partition] ready to send messages to GameServer: {}",
										   Connection.ToString().c_str());
				}
				else if (Connection.Type == InterlinkType::eProxy)
				{
					ConnectedProxies.insert(Connection);
					logger->DebugFormatted("[Partition] ready to send messages to Proxy: {}",
										   Connection.ToString().c_str());
				...
				[this](const InterLinkIdentifier &Connection)
			{
				...
				if (Connection.Type == InterlinkType::eGameServer)
				{
					...
					logger->DebugFormatted("[Partition] GameServer disconnected: {}",
										   Connection.ToString().c_str());
				}
				else if (Connection.Type == InterlinkType::eProxy)
				{
					ConnectedProxies.erase(Connection);
					logger->DebugFormatted("[Partition] Proxy disconnected: {}",
										   Connection.ToString().c_str());
					logger->DebugFormatted("[Partition] Proxy count: {}", ConnectedProxies.size());
				}
			}}});
	...
	Interlink::Get().SendMessageRaw(InterLinkIdentifier::MakeIDGod(),
									std::as_bytes(std::span(testMessage)),
...
void Partition::MessageArrived(const Connection &fromWhom, std::span<const std::byte> data)
{
	...
	if (ParseEntityPacket(data, header, entities))
	{
		logger->DebugFormatted("[Partition] Received {} entities from {} with header {}",
							   entities.size(), fromWhom.target.ToString(),
		...
		switch (header)
		{
			...
			{
				for (const auto &entity : entities) managedEntities.push_back(entity);
				break;
			}
	...
	logger->DebugFormatted("MESSAGE RECEIVED: '{}' (length: {}, header: {})", msg, msg.length(),
						   static_cast<int>(header));
	...
	switch (header)
	{
		case AtlasNetMessageHeader::EntityIncoming:
		{
			...
			{
				logger->ErrorFormatted("Invalid EntityIncoming message format (missing colon): {}",
									   msg);
				return;
			}
			...
			if (entityIdStr.find_first_not_of("0123456789") != std::string::npos)
			{
				logger->ErrorFormatted(
					"Invalid entity ID format (non-numeric): '{}' in message: {}", entityIdStr,
					msg);
				return;
			}
			...
			{
				logger->ErrorFormatted(
					"Invalid entity ID format in EntityIncoming message: '{}' - Error: {}",
					entityIdStr, e.what());
				return;
			}
			...
			{
				logger->ErrorFormatted(
					"Entity ID out of range in EntityIncoming message: '{}' - Error: {}",
					entityIdStr, e.what());
				return;
			}
			...
			if (it != outliers.end())
			{
				...
				if (!PartitionEntityManifest::AddEntity(partitionKey, *it))
				{
					logger->ErrorFormatted("Failed to add entity {} to partition manifest",
										   entityId);
				}
				else
				{
					logger->DebugFormatted(
						"PARTITION MANIFEST: Added entity {} to partition manifest", entityId);
				}
				...
				logger->DebugFormatted(
					"REDISTRIBUTION COMPLETE: Successfully fetched entity {} from {}'s outliers",
					entityId, sourcePartition);
				...
				if (it->IsPlayer)
				{
					...
					for (const auto &proxy : ConnectedProxies)
					{
						Interlink::Get().SendMessageRaw(proxy,
														std::as_bytes(std::span(authorityMsg)),
														InterlinkMessageSendFlag::eReliableNow);
						logger->DebugFormatted(
							"PROXY NOTIFY: Player {} moved to {}, sent to proxy {}", it->ID,
							partitionStr, proxy.ToString());
					}
				}
			}
			else
			{
				logger->ErrorFormatted("Could not find entity {} in {}'s outliers", entityId,
									   sourcePartition);
			}
			break;
		}
		...
		case AtlasNetMessageHeader::EntityOutgoing:
		{
			...
			try
			{
				...
				{
					logger->ErrorFormatted(
						"Invalid entity ID format in EntityOutgoing message: '{}' - Error: {}",
						entityIdStr, e.what());
					return;
				}
				catch (const std::out_of_range &e)
				{
					logger->ErrorFormatted(
						"Entity ID out of range in EntityOutgoing message: '{}' - Error: {}",
						entityIdStr, e.what());
					return;
				}
				...
				auto it =
					std::find_if(managedEntities.begin(), managedEntities.end(),
								 [entityId](const AtlasEntity &e) { return e.ID == entityId; });
				...
				if (it != managedEntities.end())
				{
					...
					logger->DebugFormatted("REMOVED: Removed entity {} from managed entities",
										   entityId);

					// Update partition manifest to remove entity
					std::string partitionKey = getCurrentPartitionId().ToString();
					if (!PartitionEntityManifest::RemoveEntity(partitionKey, entityId))
					{
						logger->ErrorFormatted("Failed to remove entity {} from partition manifest",
											   entityId);
					}
					else
					{
						logger->DebugFormatted(
							"PARTITION MANIFEST: Removed entity {} from partition manifest",
							entityId);
					}

					// Send entity info back to God for redistribution
					std::string entityInfoMsg =
						"EntityRemoved:" +
						InterLinkIdentifier(InterlinkType::eShard,
											DockerIO::Get().GetSelfContainerName())
							.ToString() +
						":" + std::to_string(entity.ID) + ":" + std::to_string(entity.Position.x) +
						":" + std::to_string(entity.Position.y) + ":" +
						std::to_string(entity.Position.z);

					Interlink::Get().SendMessageRaw(InterLinkIdentifier::MakeIDGod(),
													std::as_bytes(std::span(entityInfoMsg)),
													InterlinkMessageSendFlag::eReliableNow);
					logger->DebugFormatted("REDISTRIBUTION: Sent entity info back to God: {}",
										   entityInfoMsg);
				}
				else
				{
					logger->DebugFormatted(
						"ENTITY NOT FOUND: Entity {} not in managed entities (may have been "
					...
					std::string entityInfoMsg =
						...
						":0.0:0.0:0.0";	 // Dummy position since entity not found

					Interlink::Get().SendMessageRaw(InterLinkIdentifier::MakeIDGod(),
													std::as_bytes(std::span(entityInfoMsg)),
													InterlinkMessageSendFlag::eReliableNow);
					logger->DebugFormatted(
						"ACKNOWLEDGED: Sent dummy response to God for missing entity {}", entityId);
				}
			}
			catch (const std::exception &e)
			{
				logger->ErrorFormatted("Error parsing EntityOutgoing message: {} - Message: '{}'",
									   e.what(), msg);
			}
			break;
		}
		...
		case AtlasNetMessageHeader::FetchGridShape:
		{
			...
			std::optional<std::string> gridShapeData = GridCellManifest::Fetch(key);
			...
			try
			{
				...
				logger->DebugFormatted("Successfully loaded grid shape with {} cells",
									   partitionGridShape.cells.size());
				...
				for (size_t i = 0; i < partitionGridShape.cells.size(); ++i)
				{
					...
					logger->DebugFormatted("Grid Cell {}: ({}, {}) to ({}, {}) [row={}, col={}]", i,
										   cell.min.x, cell.min.y, cell.max.x, cell.max.y, cell.row,
										   cell.col);
				}
...
void Partition::checkForOutliersAndNotifyGod()
{
	...
	if ((partitionShape.triangles.empty() && partitionGridShape.cells.empty()) ||
	...
	for (const auto &entity : managedEntities)
	{
		...
		if (!partitionGridShape.cells.empty())
		{
			...
			if (partitionGridShape.contains(entityPos))
			{
				isInside = true;
				logger->DebugFormatted(
					"Entity {} at ({}, {}) is INSIDE grid cells during outlier check", entity.ID,
		...
		else
		{
			...
			for (const auto &triangle : partitionShape.triangles)
			{
												   ...
												   triangle[2]))
				{
					isInside = true;
					logger->DebugFormatted(
						"Entity {} at ({}, {}) is INSIDE triangle during outlier check", entity.ID,
						entityPos.x, entityPos.y);
					break;
				}
			}
		...
		if (!isInside)
		{
			...
			if (reportedOutliers.find(entity.ID) == reportedOutliers.end())
			{
				...
				logger->DebugFormatted(
					"Entity {} at ({}, {}) is OUTSIDE all shapes - marked as outlier during check",
	...
	if (!outliers.empty())
	{
		...
		logger->DebugFormatted("ATTEMPTING: Trying to store {} outliers for partition {}",
							   outliers.size(), partitionKey);
		if (EntityManifest::StoreOutliers(partitionKey, outliers))
		{
			logger->DebugFormatted("DATABASE: Stored {} outliers for partition {}", outliers.size(),
								   partitionKey);

			// Notify God about outliers
			std::string notifyMsg =
				"OutliersDetected:" + partitionKey + ":" + std::to_string(outliers.size());
			Interlink::Get().SendMessageRaw(InterLinkIdentifier::MakeIDGod(),
											std::as_bytes(std::span(notifyMsg)),
											InterlinkMessageSendFlag::eReliableNow);
		...
		{
			logger->ErrorFormatted("DATABASE: Failed to store outliers for partition {}",
								   partitionKey);
			// Try to debug the issue
			logger->ErrorFormatted("DEBUG: Partition key: {}, Outliers count: {}", partitionKey,
								   outliers.size());
			for (const auto &entity : outliers)
			{
				logger->ErrorFormatted("DEBUG: Entity {} at ({}, {})", entity.ID, entity.Position.x,
									   entity.Position.z);
			}
		}
	}
...
void Partition::notifyGodAboutOutliers()
{
	...
	auto timeSinceLastNotification =
		std::chrono::duration_cast<std::chrono::seconds>(now - lastOutlierNotification);
	...
	{
		logger->DebugFormatted(
			"PERIODIC NOTIFICATION: Sending periodic outlier notification to God for {} outliers",
			outliers.size());
		...
		{
			logger->DebugFormatted(
				"AGGRESSIVE NOTIFICATION: Sending additional outlier notification to ensure God "
				"gets the message");
...
{
	EntityManifest::PushManagedEntitiesSnapshot(getCurrentPartitionId().ToString(), managedEntities,
												lastEntitiesSnapshotPush, logger);
}
...
// ============================================================================
bool Partition::ParseEntityPacket(std::span<const std::byte> data, AtlasNetMessageHeader &outHeader,
