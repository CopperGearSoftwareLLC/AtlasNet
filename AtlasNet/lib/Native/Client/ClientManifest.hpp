...
class ClientManifest : public Singleton<ClientManifest>
{
   ...
   private:
	...
	void __ProxyRemoveClient(const NetworkIdentity& idProxy,
							 const ClientID& ID);
	void __ProxyAddClient(const NetworkIdentity& idProxy, const ClientID& ID);
	void __ProxyRemoveClients(const NetworkIdentity& idProxy,
							  const std::span<ClientID>& ID);

   public:
	void RegisterClient(const Client& client);
	void GetProxyClients(const NetworkIdentity& id,
