...
void ClientLink::OnSteamNetConnectionStatusChanged(
...
void ClientLink::ConnectToAtlasNet(const IPAddress &address)
{
	...
	HSteamNetConnection conn = SteamNetworkingSockets()->ConnectByIPAddress(
		address.ToSteamIPAddr(), 1, opt);
...
void ClientLink::OnClientIDAssignedPacket(
...
void ClientLink::ReceiveMessages()
{
	...
	int numMsgs = SteamNetworkingSockets()->ReceiveMessagesOnPollGroup(
		poll_group, pIncomingMessages, 32);
	...
	for (int i = 0; i < numMsgs; ++i)
	{
		...
		ASSERT(BySteamCon.contains(msg->m_conn),
			   "Received message from unregistered connection?");
...
void ClientLink::OnConnected(SteamNetConnectionStatusChangedCallback_t *pInfo)
{
	...
	const uint8_t *identityByteStream =
		(const uint8_t *)pInfo->m_info.m_identityRemote.GetGenericBytes(
